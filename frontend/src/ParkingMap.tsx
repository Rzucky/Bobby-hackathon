...
function ParkingMap() {
	...
	const mapCenter = {
		...
		lat: 45.807886572584735, lng: 16.00218318614229
	};
	const mapZoom = 15
	...
	const getShortestDistance = (
		...
		cb: CallableFunction
			...
			service.getDistanceMatrix({
				...
				let minIdx = -1, minDist = 9999999
			//	console.log("COUNT:: ", resp[0]?.rows[0].elements.length)
				resp[0]?.rows[0].elements.forEach((elem, idx) => {
			
					if (elem.status === google.maps.DistanceMatrixElementStatus.OK
						&& elem.distance.value <= minDist) {
	...
	const onLoad = useCallback((mapInst: google.maps.Map) => {		
		...
		btn.onclick = () => {
			...
			const ofst: number = 0.01
			const markerList: google.maps.LatLngLiteral[] = [
				/*{
			...
			const buildDistances = () => {
				getCurrLocation((loc: google.maps.LatLngLiteral) => {
					getShortestDistance(/*loc*/origg, markerList, (minIdx: number, minDist: number) => {
						// orig
						// markerList[minIdx]
	...
	// const markerComponents: ReactNode[] = markerList.map((mpos, idx) => 
	// 	<MarkerF
	// 	position={mpos}
	...
	return (
		<div style={{ height: '100vh', width: '100%' }}>
			...
			onLoad={onLoad}
			onClick={mapOnClick}
			// </div>onClick={(e)=>{console.log(e.latLng?.lat(), e.latLng?.lng())}}
			onIdle={() => {
				...
				/>
				{//<div>{markerComponents}</div>
				}

				
				{/* <MarkerF
				position={{...mapCenter, lat: mapCenter.lat + 0.05}}
				icon='pins/green_pin.png'
				onClick={(e)=>{
				...
				/>
				<MarkerF
				position={{...mapCenter, lat: mapCenter.lat + 0.1}}
				icon='pins/green_pin.png'
				onClick={(e)=>{
					console.log(`## Marker Click: `, e)
				}}
